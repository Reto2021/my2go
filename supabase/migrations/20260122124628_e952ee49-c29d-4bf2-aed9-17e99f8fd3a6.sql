-- Fix save_session_progress to not try to update duration_seconds (it's a generated column)
-- Instead, just use the passed _duration_seconds parameter directly for calculations
...
BEGIN
  ...
  -- Calculate reward based on current duration parameter (NOT the generated column)
  SELECT * INTO v_tier
  ...
  -- Update session with total reward so far (for display purposes only)
  -- Note: We do NOT update duration_seconds because it's a GENERATED column
